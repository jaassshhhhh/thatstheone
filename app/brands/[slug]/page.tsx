    ...
    <Layout>
      ...
      <div style={{ maxWidth: 680, margin: '0 auto', padding: '16px 14px 40px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: 14, marginBottom: 16 }}>
            ...
            <div style={{ flex: 1 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                ...
                {brand.velocity_delta !== 0 && brand.velocity_delta != null && (
            ...
            {(brand.website_url || brand.website) && (
              <a href={brand.website_url || brand.website} target="_blank" rel="noopener noreferrer"
                style={{ fontSize: 11, padding: '6px 12px', borderRadius: 10, background: 'rgba(99,102,241,.15)', color: '#818CF8', border: '0.5px solid rgba(99,102,241,.3)', textDecoration: 'none', whiteSpace: 'nowrap' }}>
                Visit brand →
              </a>
            )}
          </div>
          ...
          <div style={{ background: 'rgba(52,211,153,.06)', border: '0.5px solid rgba(52,211,153,.2)', borderRadius: 14, padding: '14px 16px', marginBottom: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
            ...
            {bestDeal.best_code && (
              <button onClick={() => copyCode(bestDeal.best_code, 'best')}
                style={{ fontSize: 13, fontWeight: 700, padding: '8px 16px', borderRadius: 10, background: copied === 'best' ? 'rgba(34,197,94,.2)' : 'rgba(52,211,153,.15)', color: copied === 'best' ? '#34D399' : '#34D399', border: '0.5px solid rgba(52,211,153,.3)', cursor: 'pointer', whiteSpace: 'nowrap', fontFamily: 'monospace', letterSpacing: '.06em' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {creators.map((c, i) => (
            <Link key={`${c.creator_id}`} href={`/creators/${c.creator_slug}`} style={{ textDecoration: 'none' }}>
              <div className="cc" style={{
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                    {c.best_code && (
                      <button onClick={e => { e.preventDefault(); copyCode(c.best_code, c.creator_id) }}
                    ...
                    {c.best_promo_url && !c.best_code && (
                      <a href={c.best_promo_url} target="_blank" rel="noopener noreferrer"
                        onClick={e => e.stopPropagation()}
                        style={{ fontSize: 11, padding: '5px 10px', borderRadius: 8, background: 'rgba(99,102,241,.1)', color: '#818CF8', border: '0.5px solid rgba(99,102,241,.2)', textDecoration: 'none' }}>
                        Deal →
                      </a>
                    )}
                  </div>
                ...
                {c.best_quote && (
                  <p style={{ fontSize: 11, color: 'rgba(255,255,255,.3)', margin: '10px 0 0', fontStyle: 'italic', lineHeight: 1.5, borderLeft: '2px solid rgba(255,255,255,.08)', paddingLeft: 10 }}>
                    "{c.best_quote.slice(0, 120)}{c.best_quote.length > 120 ? '...' : ''}"
                  </p>
                )}
              </div>
            </Link>
